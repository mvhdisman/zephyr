...
K_KERNEL_STACK_DEFINE(write_thrd_stack, 4096);
struct k_thread write_thrd;
k_tid_t write_tid;
...
int xenbus_probe_device(struct xenbus_driver *drv,
...
static void xenbus_read_thrd(void *p1, void *p2, void *p3)
{
    ...
    do_write_test("data/stiff", "flobble");

	// int err = 0;
	// err = xs_rm(XBT_NIL, "device/suspend");
	// if(err < 0)
	// 	printk("D:\txs_rm failed: err = %d\n",err);

}


void do_dir_test(const char *path)
{
	int err = 0;
	
	err = xs_rm(XBT_NIL, path);
	if(err < 0)
		printk("D:\txs_rm failed: err = %d\n",err);


	err = xs_mkdir(XBT_NIL, path);
	if(err < 0)
		printk("D:\txs_mkdir failed: err = %d\n",err);
}


void do_write_test(const char *path, const char *val)
{
	int err;
    printk("Write %s to %s...\n", val, path);
    err = xs_write(XBT_NIL, path, val);
    if (err < 0) {
	printk("Result %d\n", err);
    } else {
	printk("Success.\n");
...
void do_rm_test(const char *path)
{
...
static int xenbus_init(const struct device *dev)
{
	...
	// write_tid = k_thread_create(&write_thrd, write_thrd_stack,
	// 			K_KERNEL_STACK_SIZEOF(write_thrd_stack),
	// 			xenbus_write_thrd, NULL, NULL, NULL, 7, 0, K_NO_WAIT);
	// if (!write_tid) {
	// 	printk("%s: Failed to create write thread\n", __func__);
	// 	k_thread_abort(xenstore_tid);
	// 	return -1;
	// }
