...
{
	printk("before gnttab_grant_init\n");
	gnttab_grant_init(gref, domid, mfn);
	printk("after gnttab_grant_init\n");
		...
		int readonly)
{
	...
	printk("free entry grabbed\n");
	gnttab_grant_permit_access(8, domid, mfn, readonly);
...
static int gnttab_init(const struct device *d)
{
	...
	gnttab.table = (grant_entry_v1_t *)
			(to_virt(DT_REG_ADDR_BY_IDX(DT_INST(0, xen_xen), 0)));
